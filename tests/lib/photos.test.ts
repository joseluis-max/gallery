...
describe('getPhotoImageUrls', () => {
  ...
  it('derives the JPEG fallback key from the WebP key stored on the photo', () => {
    const urls = getPhotoImageUrls({ r2: { originalKey: 'x.jpg', publicKey: 'sea-lions-dock.webp' } }, storage);
  ...
  it('is case-insensitive on the .webp extension', () => {
    const urls = getPhotoImageUrls({ r2: { originalKey: 'x.jpg', publicKey: 'photo.WEBP' } }, storage);
    expect(urls.jpeg).toBe('/local-public/photo.jpg');
  });

  it('only replaces a trailing .webp, not one appearing mid-slug', () => {
    const urls = getPhotoImageUrls({ r2: { originalKey: 'x.jpg', publicKey: 'webp-launch-day.webp' } }, storage);
